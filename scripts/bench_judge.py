...
LAW_CITATION_CASES = [
    ...
    ("医疗期内,用人单位能否解除劳动合同?",
     ["劳动法", "42", "第四十二条", "不得"]),
    ("劳动者患病医疗期满不能从事原工作,解除后应支付什么?",
     ["40", "第四十条", "代通知金", "N+1"]),
    ("未签劳动合同满一年,视为订立什么合同?",
     ["14", "第十四条", "无固定期限"]),
    ("劳务派遣单位与用工单位对劳动者承担什么责任?",
     ["劳务派遣", "连带", "92", "第九十二条"]),
]

# 2. 数字蕴含: (证据文本, 期望数字集合, 是否要求派生计算)
#    needs_calculation=True 时,期望值是从证据推导的法律结果(模型必须计算,不能只复述)
NUMERIC_CASES = [
    ...
    ("被迫解除通知书:经济赔偿金90600元,2020年7月1日入职。",
     {"90600"}, False),
    # 社保缴费基数(5000)是社平 60%-300%,不等于实际工资(8000)
    ("社保参保证明:缴费基数5000元。劳动合同约定月工资8000元。",
     {"5000", "8000"}, False),
    # 公积金月缴存额(1490) vs 工资(7550)——豆包基准曾把 1490 误写为 7554
    ("个人住房公积金查询书:月缴存额1490元。工资表:月工资7550元。",
     {"1490", "7550"}, False),
    # 金额含小数与千分位(7,550.00 → 7550;355.00 → 355)
    ("银行流水:2025年1月实发工资7,550.00元,个税扣除355.00元。",
     {"7550", "355"}, False),
    # ── 派生计算(模型必须算,不能只复述) ──
    # 经济补偿:2年4个月 = 2 整年 + 4 个月(<6个月)→ 2 + 0.5 = 2.5 个月
    ("劳动合同:月工资8000元,工作2年4个月,违法解除。",
     {"2.5"}, True),
    # 高薪封顶:月薪50000 > 社平3倍(30000),封顶基数30000、年限封顶12年
    # 违法解除 2N = 30000 × 12 × 2 = 720000
...
def bench_numeric(backend: str) -> dict:
    hits = 0
    failures = []
    for evidence, expected_nums, needs_calc in NUMERIC_CASES:
        if needs_calc:
            prompt = (f"证据: {evidence}\n"
                      f"请根据证据计算法律结论中的关键数字(如经济补偿月数、赔偿金总额),"
                      f"回答中必须包含计算出的数值。")
        else:
            prompt = (f"证据: {evidence}\n"
        ...
        if len(found) == len(expected_nums):
            ...
            failures.append({"evidence": evidence, "needs_calc": needs_calc,
                             "resp": resp[:200],
