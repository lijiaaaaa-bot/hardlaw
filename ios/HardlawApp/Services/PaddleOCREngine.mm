...
static std::vector<Box> ProbToBoxes(const float* m,int H,int W,int ow,int oh,float th){
    ...
    for(int i=0;i<N;i++)mask[i]=m[i]>th?255:0;
    int minA=N/500; std::vector<bool> seen(N,false); const int dy[]={-1,0,1,0},dx[]={0,1,0,-1};
...
static NSString* CTC(const float* l,int T,int C,NSArray<NSString*>* d){
...
- (instancetype)initWithDetModel:(NSString*)dp detParams:(NSString*)dpp
                       recModel:(NSString*)rp recParams:(NSString*)rpp dictPath:(NSString*)dict {
...
- (NSArray<NSString*>*)recognize:(CGImageRef)img error:(NSError**)err {
    if(!_isLoaded){if(err)*err=[NSError errorWithDomain:@"PaddleOCR" code:-1 userInfo:nil];return @[];}
    ...
    for(auto& b:boxes){if(b.w<4||b.h<4)continue;CGRect cr=CGRectMake(b.x,b.y,b.w,b.h);
        ...
        if(rs.size()>=3){int T=(int)rs[1],C=(int)rs[2];
            ...
            NSString* t=CTC(rd.data(),T,C,_dict);if(t.length)[r addObject:t];}}
    return r;
}
@end
...
- (instancetype)initWithDetModel:(NSString*)dp detParams:(NSString*)dpp
                       recModel:(NSString*)rp recParams:(NSString*)rpp dictPath:(NSString*)dict {
    if(!(self=[super init]))return nil;
    _isLoaded=NO;NSLog(@"[PaddleOCR] Unavailable on simulator");
    return self;
}
- (NSArray<NSString*>*)recognize:(CGImageRef)img error:(NSError**)err {
    if(err)*err=[NSError errorWithDomain:@"PaddleOCR" code:-1 userInfo:@{NSLocalizedDescriptionKey:@"PaddleOCR only available on device"}];
    return @[];
}
@end
#endif